...
// ============================================================
// SISTEMA "VER MAIS" RESPONSIVO - CORRIGIDO
// ============================================================
...
function loadProductsToGrid(productsArray, gridElement) {
  ...
  console.log(`🔄 Carregando ${productsToShow.length} produtos (mostrando todos: ${showingAllProducts})`);
  console.log(`📦 Produtos IDs a mostrar: ${productsToShow.map(p => p.id).join(', ')}`);
...
// ============================================================
// CARREGAR PRODUTOS NA LOJA (ATUALIZADO E CORRIGIDO)
// ============================================================
function loadStoreProducts() {
  ...
  // Determinar quantos produtos mostrar
  const initialCount = getInitialProductCount();
  const productsToShow = showingAllProducts 
    ? produtosVRTIGO 
    : produtosVRTIGO.slice(0, initialCount);
  
  console.log(`🏪 Carregando ${productsToShow.length} produtos na loja`);
  console.log(`🔢 Total produtos: ${produtosVRTIGO.length}, Inicial: ${initialCount}, Mostrando tudo: ${showingAllProducts}`);
  ...
  if (!showingAllProducts && getRemainingProductsCount() > 0) {
    ...
    const existingBtn = productsGrid.parentNode.querySelector('.view-more-btn');
    if (existingBtn && existingBtn !== viewMoreBtn) {
      existingBtn.remove();
    }
    
    // Verificar se o botão já está no DOM
    if (!viewMoreBtn.parentNode) {
    ...
    console.log(`➕ Botão "Ver mais" mostrando: +${getRemainingProductsCount()} produtos`);
  } else if (viewMoreButton) {
    viewMoreButton.style.display = 'none';
    console.log('➖ Botão "Ver mais" ocultado (todos produtos visíveis)');
  }
  
  // Atualizar classe para CSS
  if (showingAllProducts) {
    productsGrid.classList.add('showing-all');
  } else {
    productsGrid.classList.remove('showing-all');
  }
}
...
function loadSavedProducts() {
  ...
  savesGrid.innerHTML = savedProducts.map(generateProductCard).join('');
  initSaveButtons(savesGrid);
  initProductModal();
}
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  initNavigation();
  loadStoreProducts(); // ← Agora com sistema "Ver mais" corrigido
  initFAQModal();
  initContactForm();
  initUIInteractions();
...
// ============================================================
// LIGHT/DARK THEME TOGGLE - CORRIGIDO
// ============================================================
...
function initThemeToggle() {
  ...
  if (savedTheme === 'light' || (!savedTheme && !systemPrefersDark)) {
    ...
    console.log('🌞 Tema claro aplicado inicialmente');
  } else {
    console.log('🌙 Tema escuro aplicado inicialmente');
  }
  
  // Alternar tema
  themeToggleBtn.addEventListener('click', () => {
    const currentTheme = document.documentElement.getAttribute('data-theme');
    console.log(`🔄 Alternando tema, atual: ${currentTheme || 'dark'}`);
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  initContactForm();
  initUIInteractions();
  initThemeToggle(); // <-- AGORA FUNCIONANDO
  updateSavesCount();
